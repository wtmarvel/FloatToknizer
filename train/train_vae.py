...
from train.loss import VAELoss
...
class TrainProcess(DistributedTrainerBase):
    ...
    def set_model_and_loss(self):
        ...
        self.loss_fn = VAELoss(kl_weight=self.opt.kl_weight).to(self.device)
    ...
    def do_optimize(self, mb, step, scaler):
        ...
        with autocast(enabled=self.opt.enable_amp):
            recon, mu, log_var = self.model(mb['value'])
            # Calculate all losses using the loss class
            mb_losses = self.loss_fn(recon, mb['value'], mu, log_var)
    ...
    def test(self, step):
        ...
        for mb in self.test_loader:
            ...
            B = mb['value'].shape[0]
            with autocast(enabled=self.opt.enable_amp):
                # Model returns tuple of (recon, mu, log_var)
                recon, mu, log_var = self.model(mb['value'])
